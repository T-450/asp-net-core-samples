...
public class FileHelpers
{
    ...
    private static readonly Dictionary<string, List<byte[]>> _fileSignatures = new FileSignatures().Signatures;
        ...
        MemberInfo property =
            typeof(T).GetProperty(
                formFile.Name.Substring(formFile.Name.IndexOf(".",
                    StringComparison.Ordinal) + 1)) ?? throw new InvalidOperationException();
        ...
        ModelStateDictionary modelState, IEnumerable<string> permittedExtensions, long sizeLimit)
    {
        ...
        {
            using var memoryStream = new MemoryStream();
            await section.Body.CopyToAsync(memoryStream).ConfigureAwait(false);
            switch (memoryStream)
            {
                case var stream when stream.Length == 0:
                    modelState.AddModelError("File", "The file is empty.");
                    break;
                case var stream when stream.Length > sizeLimit:
                    modelState.AddModelError("File", $"The file exceeds {sizeLimit / 1048576:N1} MB.");
                    break;
                case var stream
                    when !IsValidFileExtensionAndSignature(contentDisposition.FileName.Value, stream,
                        permittedExtensions):
    ...
    public static bool IsValidFileExtensionAndSignature(string fileName, MemoryStream? data,
        IEnumerable<string> permittedExtensions)
    {
        if (string.IsNullOrEmpty(fileName) || data == null || data.Length == 0)
        {
    ...
    public readonly struct FileSignatures
    {
        ...
        public Dictionary<string, List<byte[]>> Signatures { get; } = new()
        {
